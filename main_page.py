...
st.write("A esta hora ayer hacía",str(temp_ayer), "grados, y mañana se esperan", str(temp_mañana), "+/-",desv_temp)
...
def plot_prec_data(data):
        ...
        for column in data.columns[:-1]:
            ax.plot(data.index, data[column], alpha=0.9)
            ax.plot(data.index,data[column].cumsum(),alpha=0.3,linestyle="--")
